Ext.define('Mobproj.view.smartdevice.login.LoginController', {
	extend : 'Ext.app.ViewController',
	alias : 'controller.login',
	//views : [ "Login" ],
	requires : ['Mobproj.view.smartdevice.design.drawer.main.MainPanel',
	'Mobproj.view.smartdevice.login.ForgetPassword'],
			...
			Ext.Ajax.request({
				...
				success : function(response, scope) {
					...
					if (jsonResponse.response.success) {
						...
						var userinfo = Ext.decode(jsonResponse.response.userinfo);
...
/*	,onSessionLoginClick : function() {
		...
		if (form.isValid()) {
			...
			Ext.Ajax.request({
				url : "http://digitization.cloudapp.net:8080/Mobproj/secure/Authentication/authenticate",
				method : 'POST',
						...
						if (jsonResponse.response.changePassword == true) {
							Ext.Msg.confirm('Confirm', jsonResponse.response.message, function(id, value) {
								if (id == 'yes') {
									...
									scope.container.removeAll();
									scope.container.add(Ext.create('Mobproj.view.smartdevice.login.ChangePasswordScreen'));
								}
							}, {
								controller : this,
								scope : scope
	...
	onForgetPasswordClick : function(btn, opts) {
		...
		Ext.Ajax.request({
			timeout : 180000,
			url : "http://digitization.cloudapp.net:8080/Mobproj/secure/PasswordGenerator/findSecurityQuestions",
			...
			success : function(response, sender) {
				...
				if (responseText.response.success) {
					...
					var forgetPasswordWindow = Ext.create('Mobproj.view.smartdevice.login.ForgetPasswordWindow', {
